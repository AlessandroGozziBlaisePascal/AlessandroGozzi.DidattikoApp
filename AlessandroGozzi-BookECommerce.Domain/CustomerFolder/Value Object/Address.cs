    ...
    public record Address
    {
        ...
        public static Result<Address> Create(string street, string civicN, string city, string cap)
        {
            ...
            if (string.IsNullOrWhiteSpace(civicN))
                return Result.Failure<Address>(new Error("Address civic number", "Street cannot be null", ErrorType.Validation));
            if (string.IsNullOrWhiteSpace(city))
                return Result.Failure<Address>(new Error("Address city", "Street cannot be null", ErrorType.Validation));
